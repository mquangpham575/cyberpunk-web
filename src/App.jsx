...
const useCart = () => {
  ...
  // Memoize total to prevent unnecessary recalculations on re-renders
  const total = useMemo(() => {
    return cart.reduce((acc, item) => {
      if (item.price === "???") return acc;
      // Sanitize price string (remove commas) and parse
...
/**
 * Header component containing Navigation, Logo, and Cart Dropdown.
 */
  ...
  return (
    <header className="w-full container mx-auto px-6 md:px-12 pt-6 md:pt-8 flex justify-between items-center z-50">
      ...
      <div className="flex items-center gap-4">
        ...
        <button
          onClick={toggleAudio}
          className={`flex items-center gap-1 px-3 py-1 border rounded-sm transition-all duration-300 ${
            !isMuted
        ...
        >
          {!isMuted ? (
            <>
              ...
              <span className="font-mono text-[10px] font-bold">BGM: ON</span>
            </>
          ) : (
            <>
              <VolumeX size={14} />
